...
import { useSets } from '@/hooks/useApiHooks/useSetsApiData'
...
export function SetsSection() {
  ...
  if (error) {
    return (
      <section className="relative min-h-screen bg-zinc-950 py-20 px-4">
        <div className="relative z-10 h-full max-w-7xl mx-auto flex flex-col items-center justify-center">
          <div className="text-red-500 text-xl">Error al cargar los sets</div>
        </div>
      </section>
    )
  }
              ...
              >
                <div className="relative aspect-square rounded-2xl overflow-hidden bg-zinc-900 p-8 flex flex-col justify-between">
                  <div>
                    ...
                    {filteredSets[currentIndex].djs && (
                      <p className="text-zinc-400 text-xl font-medium mb-2">
                        {filteredSets[currentIndex].djs.map(dj => dj.name).join(', ')}
                      </p>
                    )}
